...
func ProveBinary(publicKey *edwards25519.Point, ct *elgamal.Ciphertext, m int, r *edwards25519.Scalar) *BinaryProof {
	return ProveBinaryWithDomain(binaryDomain, publicKey, ct, m, r)
}
...
func VerifyBinary(publicKey *edwards25519.Point, ct *elgamal.Ciphertext, p *BinaryProof) bool {
	return VerifyBinaryWithDomain(binaryDomain, publicKey, ct, p)
}

// ProveBinaryWithDomain creates a ZK proof that ct encrypts m ∈ {0, 1}
// using the given domain separator for Fiat-Shamir. This allows reusing
// the disjunctive Chaum-Pedersen structure for different proof contexts
// (e.g., candidate sum proofs, consistency proofs) without cross-protocol
// challenge collisions.
func ProveBinaryWithDomain(domain string, publicKey *edwards25519.Point, ct *elgamal.Ciphertext, m int, r *edwards25519.Scalar) *BinaryProof {
	g := edwards25519.NewGeneratorPoint()
	if m == 0 {
		return proveBinaryReal0(domain, g, publicKey, ct, r)
	}
	return proveBinaryReal1(domain, g, publicKey, ct, r)
}

// VerifyBinaryWithDomain verifies a binary proof using the given domain separator.
func VerifyBinaryWithDomain(domain string, publicKey *edwards25519.Point, ct *elgamal.Ciphertext, p *BinaryProof) bool {
	g := edwards25519.NewGeneratorPoint()

	e := fiatShamirBinary(domain, publicKey, ct, p.A0, p.B0, p.A1, p.B1)
...
func fiatShamirBinary(domain string, publicKey *edwards25519.Point, ct *elgamal.Ciphertext, a0, b0, a1, b1 *edwards25519.Point) *edwards25519.Scalar {
	return internal.FiatShamir(domain,
...
func proveBinaryReal0(domain string, g, h *edwards25519.Point, ct *elgamal.Ciphertext, r *edwards25519.Scalar) *BinaryProof {
	...
	e := fiatShamirBinary(domain, h, ct, a0, b0, a1, b1)
...
func proveBinaryReal1(domain string, g, h *edwards25519.Point, ct *elgamal.Ciphertext, r *edwards25519.Scalar) *BinaryProof {
	// Simulate branch 0
