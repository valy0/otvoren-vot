...
func (h *CeremonyHandler) runTally(ctx context.Context, cs *CeremonyState) {
	...
	allBallots, err := h.bbClient.FetchAllBallots(ctx)
	if err != nil {
	...
	// Filter to active set only — exclude overridden ballots
	var ballots []ceremony.SerializedBallot
	if len(cs.ActiveSet) > 0 {
		activeIDs := make(map[string]bool, len(cs.ActiveSet))
		for _, id := range cs.ActiveSet {
			activeIDs[id] = true
		}
		for _, b := range allBallots {
			if activeIDs[b.BallotID] {
				ballots = append(ballots, b)
			}
		}
		slog.Info("filtered to active set", "ceremony", cs.ID, "total", len(allBallots), "active", len(ballots))
	} else {
		ballots = allBallots
		slog.Warn("no active set provided — tallying all ballots", "ceremony", cs.ID)
	}

	if len(ballots) == 0 {
		slog.Error("no ballots in active set", "ceremony", cs.ID)
		h.mu.Lock()
		_ = cs.SetPhase(PhaseError)
		h.mu.Unlock()
		return
