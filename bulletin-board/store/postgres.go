...
func (s *Store) RunMigrations(ctx context.Context) error {
	...
	// Acquire a dedicated connection for advisory lock consistency.
	// Advisory locks are per-connection, so we must hold the same connection
	// for lock, all migration work, and unlock.
	...
	for _, entry := range entries {
		...
		if err := s.runMigration(ctx, conn, name, sql); err != nil {
			return err
		}
	}
	return nil
...
// runMigration executes a single migration file inside a transaction on the given connection.
func (s *Store) runMigration(ctx context.Context, conn *pgxpool.Conn, name string, sql []byte) error {
