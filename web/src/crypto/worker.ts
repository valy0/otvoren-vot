...
export type WorkerResponse =
  | {
      ...
      zkProofs: { binary_party: SerializedBinaryProof[]; sum_one: SerializedSumOneProof }
    }
  | {
      type: 'encrypt-error'
      message: string
...
self.onmessage = async (event: MessageEvent<WorkerRequest>) => {
  ...
  try {
    ...
    const encResults = Array.from({ length: numParties }, (_, i) =>
    ...
    const response: WorkerResponse = {
      ...
      encryptedBallot: {
        ...
        candidate_vectors: [],  // deferred (proofs 4-5)
      },
      zkProofs: {
